...
const createBlog = async function (req, res) {
	...
	console.log(blogFile)
        ...
        return res.status(201).send({ status: true,message:"success", data: blogCreated })
    } catch (error) {
...
const getFilteredBlog = async function (req, res) {
    try {
        ...
        let Blog = await blogModel.find({...queryParams })
        if (Blog) {
           return res.status(200).send({ status:true, message: "Done",data:Blog })
        } else {
           return res.status(404).send({ status:false, message: "Document doesnt exist" })
        }
    }
    catch (err) {
